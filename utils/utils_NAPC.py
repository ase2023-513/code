...
def inv_inception_normalization_tf(image):
  return tf.clip_by_value((image + 1.0) * 0.5 * 255,0,255)
...
inv_normalization_fn_map = {
    ...
    'mobilenet_v1': inv_inception_normalization,
}
...
checkpoint_paths = {
    ...
    #'inception_v3': base_path+'/model.ckpt-294590',
...
def load_two_image(image_path, image_path2, image_size, batch_size):
    images = []
    images2 = []
    filenames=[]
    labels=[]
    ...
    for i,filename in enumerate(files):
        ...
        image=np.array(image)
        images.append(image)
        image=Image.open(image_path2 + filename)
        image=image.resize((image_size,image_size))
        image=np.array(image)
        ...
        idx+=1
        if idx==batch_size:
            yield np.array(images),np.array(images2)
            idx=0
            images=[]
            images2 = []
            filenames=[]
