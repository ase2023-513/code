...
def gaussian(x, shape):
    x_gaussian = x+ np.random.normal(0,0.2,size=shape)
    return x_gaussian
model_names=['vgg_16']
...
def verify(model_name,ori_image_path,adv_image_path):
    ...
    x = image_preprocessing_fn(utils.inv_inception_normalization_tf(gaussian(utils.inception_normalization(image_ph),shape=[batch_size,image_size,image_size,3])))
