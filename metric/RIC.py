...
def jpeg_compress(xs, x_min=-1.0, x_max=1.0, quality=50):
    @tf.custom_gradient
...
def verify(model_name,ori_image_path,adv_image_path):
    ...
    x = image_preprocessing_fn(utils.inv_inception_normalization_tf(jpeg_compress(utils.inception_normalization(image_ph))))
